...
export async function deleteNote(noteId, { organisationId, contactId } = {}) {
  ...
  if (organisationId) {
    await pool.query(`DELETE FROM crm_notes WHERE note_id = $1 AND organisation_id = $2`, [noteId, organisationId]);
  } else if (contactId) {
    await pool.query(`DELETE FROM crm_notes WHERE note_id = $1 AND contact_id = $2`, [noteId, contactId]);
  }
}
